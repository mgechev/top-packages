...
const findAngularPackages = (startDir: string) => {
  ...
  const dirContent = readdirSync(startDir);
  if (!dirContent.includes('package.json')) {
    return;
  }
  const pkg = JSON.parse(readFileSync(join(startDir, 'package.json')).toString());
  const packageName = pkg.name;
  const packageVersion = pkg.version;
  const peerDeps = Object.keys(pkg.peerDependencies || {}).filter((key: string) => {
...
writeFileSync('angular-packages.json', JSON.stringify(angularPackages, null, 2));
